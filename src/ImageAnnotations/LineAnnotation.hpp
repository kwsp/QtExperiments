...
public:
    LineAnnotation(const QPointF &start, const QPointF &end, const QColor &color, QGraphicsItem *parent = nullptr)
        : QGraphicsItem(parent), startPoint(start), endPoint(end), lineColor(color) {
...
public:
    BoxAnnotation(const QRectF &rect, const QColor &color, QGraphicsItem *parent = nullptr)
